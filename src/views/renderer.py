...
class Renderer:
    ...
    def draw_hand(
        ...
        for i, card in enumerate(hand.cards):
            ...
            # Solo la primera carta del dealer se oculta
            # si hide_first_card es True.
    ...
    def render_all(
        ...
        # Se dibujan las manos (ocultando la primera carta
        # del dealer si es el turno del jugador).
        ...
        self.draw_text(f"Puntaje del jugador: {player_score}", (10, 530))

        # Mostrar puntaje del dealer solo si no estamos
        # ocultando su primera carta.
        if not hide_dealer_card:
            dealer_score = dealer.get_score()
            self.draw_text(f"Puntaje del dealer: {dealer_score}", (10, 200))

        # Mostrar instrucciones según el estado del juego
        if game_state == "player_turn":
            self.draw_text(
                "Presiona H para pedir carta, S para plantarte",
                (10, 570)
            )
        elif game_state == "game_over":
            # Mostrar resultado
            if winner == "player":
                self.draw_text("¡Has ganado!", (190, 350), (0, 255, 0))
            elif winner == "dealer":
                self.draw_text("Has perdido", (190, 350), (255, 0, 0))
            else:
                self.draw_text("Empate", (200, 350), (255, 255, 0))

            self.draw_text("Presiona R para jugar otra ronda", (10, 570))
